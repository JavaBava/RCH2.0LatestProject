...
@Component({
  selector: 'app-ecreport',
  templateUrl: './ecreport.component.html',
  styleUrls: ['./ecreport.component.css']
})
export class ECReportComponent implements OnInit {
...
getecdata(){
 ...
 else
 {
  ...
  this.vcode=this.selectedVillage;
    ...
    this.backendApiService.getECDataAPi(this.selectedDistrict,this.bid,this.phcid,this.subphcid,this.vcode,this.selectdYear)
...
downloadpdf(){
 ...
 else
 {
  ...
  var urlstr ="http://164.100.61.97/Home/DownloadECReport?dcode="+this.selectedDistrict+"&bid="+Bid+"&phcid="+Phcid+"&subphcid="+Subphcid+"&vcode="+Vcode+"&fin_year="+this.selectdYear;
  console.log(urlstr);
